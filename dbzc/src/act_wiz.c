...
char    * format_pl	( unsigned long long powerlevel )
{
...
if ( !str_cmp(arg, "clear") )
{
   ...
   if ( 0==remove( fname ) )
   {
      send_to_char( "Ok. Your watch file has been cleared.\n\r", ch);
      return;
   }
   send_to_char( "You have no valid watch file to clear.\n\r", ch);
   return;
}
...
if ( !str_cmp(arg, "size") )
{
   ...
   ch_printf( ch, "You have %d lines in your watch file.\n\r", rec_count);
   fclose(fp);
   return;
}
...
{
   int cou = 0;
   ch_printf( ch, "%-3s %-12s %-14s %-15s\n\r",
     " ", "Imm Name", "Player/Command", "Player Site");
   if (first_watch)
      for ( pw = first_watch; pw; pw = pw->next )
          if ( !str_cmp(ch->name, pw->imm_name) )
             ch_printf( ch, "%3d %-12s %-14s %-15s\n\r",
...
if ( !str_cmp(arg, "delete") && isdigit(*arg2) )
{
   ...
   send_to_char ("Sorry. I found nothing to delete.\n\r", ch);
   return;
}
...
if ( !str_cmp(arg, "player") && *arg2 )
{
   ...
   if (first_watch)                            /* check for dups */
	  ...
	  && pw->target_name
          ...
          {
             send_to_char("You are already watching that player.\n\r", ch);
             return;
          }
  ...
  /* so I do a str_cmp to make sure it finds the right player --Gorog */
   ...
   if ( (vic = get_char_world( ch, buf)) )    /* if vic is in game now */
      if ( (!IS_NPC(vic)) && !str_cmp(arg2, vic->name) )
         xSET_BIT( vic->pcdata->flags, PCFLAG_WATCH );
   ...
   save_watchlist( );
   send_to_char("Ok. That player will be watched.\n\r", ch);
   return;
}
...
if ( !str_cmp(arg, "site") && *arg2 )
{
   ...
   if (first_watch)                            /* check for dups */
	  ...
	  &&   pw->player_site
          ...
          {
             send_to_char("You are already watching that site.\n\r", ch);
             return;
          }
   ...
   save_watchlist( );
   send_to_char("Ok. That site will be watched.\n\r", ch);
   return;
}
...
if(!str_cmp(arg, "command") && *arg2)
{
	...
	for(pw = first_watch; pw; pw = pw->next)
	{
		if(!str_cmp(ch->name, pw->imm_name)
			...
			&& !str_cmp(arg2, pw->target_name))
		{
			send_to_char("You are already watching that command.\n\r", ch);
			return;
		}
	}
	...
	if(!found)
	{
		send_to_char("No such command exists.\n\r", ch);
		return;
	}
	else
	{
	...
	for(pw = first_watch; pw; pw = pw->next)
	{
		if(strcmp(pinsert->imm_name, pw->imm_name) < 0)
		{
			INSERT(pinsert, pw, first_watch, next, prev);
			save_watchlist();
			send_to_char("Ok, That command will be watched.\n\r", ch);
			return;
		}
	}
...
send_to_char("Sorry. I can't do anything with that. "
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
     ...
     for ( curr_lvl = LEVEL_IMMORTAL; curr_lvl <= get_trust( ch );
           curr_lvl++)
     {
         send_to_char( "\n\r\n\r", ch);
         col = 1;
         ch_printf( ch, "&z-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-<[&wLEVEL %-2d&z]>-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n\r", curr_lvl);
         for (hash = 0; hash < 126; hash++ )
             for ( cmd = command_hash[hash]; cmd; cmd = cmd->next )
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
     {
        ...
        "You are authorized to explore Dragon Ball Z: Chronicles\n\r", victim->name);
...
/*  smash_tilde( argument );
  ...
  if ( !str_cmp( argument, "none" ) && !IS_IMMORTAL(ch))
  {
  sprintf( buf, "&w&W%-12.12s&Y", capitalize(npc_race[ch->race]));
  ch->pcdata->rank = str_dup(buf);
  }
  else if ( !str_cmp( argument, "none" ) && IS_IMMORTAL(ch))
  {
  if (ch->level == 64)
  sprintf( buf, "&WMUD Owner&Y");
  if (ch->level == 63)
  sprintf( buf, "&WMUD Co-Owner&Y");
  if (ch->level == 62)
  sprintf( buf, "&WSupervisor&Y");
  if (ch->level == 61)
  sprintf( buf, "&WHead Builder");
  if (ch->level == 60)
  sprintf( buf, "&WHead RP");
  if (ch->level == 59)
  sprintf( buf, "&WHead Enforcer");
  if (ch->level == 58)
  sprintf( buf, "&WAncient Builder");
  if (ch->level == 57)
  sprintf( buf, "&WGreater Builder");
  if (ch->level == 56)
  sprintf( buf, "&WLesser Builder");
  if (ch->level == 55)
  sprintf( buf, "&WClan Manager");
  if (ch->level == 54)
  sprintf( buf, "&WRP Specialist");
  if (ch->level == 53)
  sprintf( buf, "&WSenior Builder");
  if (ch->level == 52)
  sprintf( buf, "&WJunior Builder");
  if (ch->level == 51)
  sprintf( buf, "&WTrainee Builder");
  if (ch->level == 50)
  sprintf( buf, "&WPlayer Tester");
  if (ch->level == 49)
  sprintf( buf, "&WRP Assistant");
  sprintf(buf, "&w&W%-12.12s&Y", buf);
  ch->pcdata->rank = str_dup(buf2);
  }
  else 
  { 
...
void do_transfer( CHAR_DATA *ch, char *argument )
{
   ...
   CHAR_DATA *victim;
	...
	if ( room_is_private( ch, location ) && get_trust( ch ) < sysdata.level_override_private && !IN_HOUSE(ch) )
	{
    ...
    if ( victim == ch && ch->level < 65 )
       {
        send_to_char( "Transfer your self? No.\n\r", ch );
        return;
     }
    ...
    if ( victim->in_room->vnum == 98 && ch->level < 65 )
    {
        send_to_char( "For security reasons, you can't do that.\n\r", ch );
        return;
    }
    ...
    &&   xIS_SET(victim->pcdata->flags, PCFLAG_DND) ) 
    {
       ...
       ch_printf( victim, "&RYour DND flag just foiled %s's transfer command.\n\r", ch->name);
       return;
    }
    /* end of modification                                         */
    ...
    &&   !in_hard_range( victim, location->area ) )
      act( AT_DANGER, "", ch, NULL, NULL, TO_CHAR );
}
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->in_room != location && room_is_private( ch, location ) && !IN_HOUSE(ch) )
    {
	if ( get_trust( ch ) < LEVEL_GREATER )
	{
	    send_to_char( "That room is private right now.\n\r", ch );
	...
	{
	  send_to_char( " ", ch );
	  one_argument( capitalize(rch->name), buf );
	  send_to_char( buf, ch );
	}
    }
    ...
    if ( location->clan )
    {
      ...
      clan = get_clan_short(location->clan);
      ch_printf( ch, "&cClan:&w %s\n\r", clan ? clan->name : "Invalid" );
    }
...
int x;
	...
	******/
	...
	case ITEM_LIGHT:
		...
		break;
        case ITEM_CHIP:
            ch_printf( ch, "&cValue[&W3&c]  Grade: &W%s (%d)\n\r", get_grade( obj->value[3] ), obj->value[3]);
 		break;
	case ITEM_POTION:
	case ITEM_PILL:
	case ITEM_SCROLL:
	...
	case ITEM_STAFF:
		...
		break;
	case ITEM_WEAPON:
            ch_printf( ch, "&cValue[&W0&c]   Type: &W%s (%d)\n\r", get_weapontype(obj->value[0]), obj->value[0]);
            ch_printf( ch, "&cValue[&W1&c] P.Atk.: &W%d\n\r", obj->value[1]);
            ch_printf( ch, "&cValue[&W2&c] M.Atk.: &W%d\n\r", obj->value[2]);
            ch_printf( ch, "&cValue[&W3&c]  Grade: &W%s (%d)\n\r", get_grade( obj->value[3] ), obj->value[3]);
            ch_printf( ch, "&cValue[&W4&c] Status: &W%d\n\r", obj->value[4]);
	    ch_printf( ch, "&cValue[&W5&c] AtkSpd: &W%s (%d)\n\r ", get_weaponspd( obj->value[5] ), obj->value[5]);
	    break;
	case ITEM_ARMOR:
            ch_printf( ch, "&cValue[&W0&c]   Type: &W%s (%d)\n\r", get_armortype(obj->value[0]), obj->value[0]);
            ch_printf( ch, "&cValue[&W1&c] P.def.: &W%d\n\r", obj->value[1]);
            ch_printf( ch, "&cValue[&W2&c] M.def.: &W%d\n\r", obj->value[2]);
            ch_printf( ch, "&cValue[&W3&c]  Grade: &W%s (%d)\n\r", get_grade( obj->value[3] ), obj->value[3]);
            ch_printf( ch, "&cValue[&W4&c] Status: &W%d\n\r", obj->value[4]);
            ch_printf( ch, "\n\r");
            break;
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "&cShipRoom: &C%d\n\r", obj->value[0]);
    ...
    ch_printf_color( ch, "&cX: &w%d   &cY: &w%d\n\r", obj->x, obj->y );
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    CHAR_DATA *victim;
 // int x;  -Didnt bother looking through it, wtf was this suposed to be used in -Vegeta    
    ...
    if ( !IS_NPC(victim) )
	ch_printf_color( ch, "&cStatus : &w%-10s", CAN_PKILL(victim) ? "Deadly" :
	    IS_PKILL(victim) ? "Pre-Deadly" : "Non-Deadly" );
    if ( !IS_NPC(victim) && victim->pcdata->clan )
	ch_printf_color( ch, "   &cClan: &w%s", victim->pcdata->clan->name );
    send_to_char( "\n\r", ch );
    if ( get_trust(ch) >= LEVEL_GOD && !IS_NPC(victim) && victim->desc )
	ch_printf_color( ch, "&cIP: &w@%s   Descriptor: %d  &cTrust: &w%d  &cAuthBy: &w%s\n\r",
	...
	ch_printf_color( ch, "&cHelled until %24.24s by %s.\n\r",
		ctime(&victim->pcdata->release_date),
		victim->pcdata->helled_by);
    if ( !IS_NPC(victim) && victim->pcdata->silenced_date != 0 )
        ch_printf_color( ch, "&cSilenced until %24.24s by %s.\n\r",
                ctime(&victim->pcdata->silenced_date),
                victim->pcdata->silenced_by);
    ch_printf_color( ch, "&cVnum: &w%-5d    &cSex: &w%-6s    &cRoom: &w%-5d    &cCount: &w%d   &cKilled: &w%d\n\r",
	...
	IS_NPC(victim) ? victim->pIndexData->killed
       ...
       if ( ch->level >= 65 && !IS_NPC(victim))
       {
        ...
        ch_printf_color( ch, "&cStr: &C%2d&c )( Int: &C%2d&c )( Wis: &C%2d&c )( Dex: &C%2d&c )( Con: &C%2d&c )( Cha: &C%2d&c )( Lck: &C%2d&c\n\r",
	get_curr_str(victim),
	get_curr_int(victim),
	get_curr_wis(victim),
	get_curr_dex(victim),
    ...
    ch_printf_color( ch, "&cClass  : &w%-2.2d/%-10s   &cRace      : &w%-2.2d/%-10s\n\r",
	victim->class,
    ...
    sprintf( mnbuf, "%ld/%ld", victim->mana, victim->max_mana );
    sprintf( mvbuf, "%d/%d", victim->move, victim->max_move );
    ch_printf_color( ch, "&cLife    : &w%-12s    &cKi     : &w%-12s    &cMove      : &w%-12s\n\r",	hpbuf, mnbuf, mvbuf );
    ch_printf_color( ch,  "&cHitroll : &C%-5d           &cAlign  : &w%-5d           &cArmorClass: &w%d\n\r",
	GET_HITROLL(victim), victim->alignment, get_curr_armor(victim) );
    ch_printf_color( ch, "&cDamroll : &C%-5d           &cWimpy  : &w%-5d           &cPosition  : &w%d\n\r",
	GET_DAMROLL(victim), victim->wimpy, victim->position );
    ch_printf_color( ch, "&cFighting: &w%-13s   &cMaster : &w%-13s   &cLeader    : &w%s\n\r",
	victim->fighting ? victim->fighting->who->name : "(none)",
        victim->master      ? victim->master->name   : "(none)",
        victim->leader      ? victim->leader->name   : "(none)" );
    if ( IS_NPC(victim) )
      ch_printf_color( ch, "&cHating  : &w%-13s   &cHunting: &w%-13s   &cFearing   : &w%s\n\r",
	victim->hating ? victim->hating->name : "(none)",
	victim->hunting ? victim->hunting->name : "(none)",
        victim->fearing ? victim->fearing->name : "(none)" );
    else
      ch_printf_color( ch, "&cBless     : &w%-d\n\r",   victim->pcdata->bless);

    if ( IS_NPC( victim ) )
      ch_printf_color( ch, "&cMob hitdie : &C%dd%d+%d    &cMob damdie : &C%dd%d+%3d    &cIndex damdie : &C%dd%d+%3d\n\r&cNumAttacks : &C%d\n\r",
       ...
       victim->numattacks );
    ch_printf( ch, "&cPdef: &w%-4d  &cPAtk: &w%-4d  &cEdef: &w%-4d  &cEAtk: &w%-4d", 
	get_curr_pdef( victim ), get_curr_patk( victim ), get_curr_edef( victim ), get_curr_eatk( victim ) );
    ch_printf( ch, "&cEvsn: &w%-4d  &cAccu: &w%-4d  &cCrit: &w%-4d\n\r",
        get_curr_evasion( victim ), get_curr_accuracy( victim ), get_curr_critical( victim ) );
    ch_printf( ch, "&cPdef: &w%-4d  &cPAtk: &w%-4d  &cEdef: &w%-4d  &cEAtk: &w%-4d\n\r", victim->pdef, victim->patk, victim->edef, victim->eatk );

    if ( !IS_NPC( victim ) )
      ch_printf_color( ch, "&cDrunk: &w%d\n\r",victim->drunk );
    else
      send_to_char( "\n\r", ch );
    ch_printf_color( ch, "&cItems: &w(%d/%d)  &cWeight &w(%d/%d)\n\r",
		victim->carry_number,
		can_carry_n(victim),
		victim->carry_weight,
		can_carry_w(victim) );
    ch_printf_color( ch, "&cYear: &w%-5d  &cSecs: &w%d  &cTimer: &w%d  &cGold: &Y%lld &cBank: &Y%lld\n\r",
	get_age( victim ), (int) victim->played, victim->timer, victim->gold, victim->balance );
    if ( get_timer( victim, TIMER_PKILLED ) )
	ch_printf_color( ch, "&cTimerPkilled:  &R%d\n\r",
	  get_timer( victim, TIMER_PKILLED ) );
    if ( get_timer( victim, TIMER_RECENTFIGHT ) )
	ch_printf_color( ch, "&cTimerRecentfight:  &R%d\n\r",
	  get_timer( victim, TIMER_RECENTFIGHT ) );
    if ( get_timer( victim, TIMER_ASUPRESSED ) )
	ch_printf_color( ch, "&cTimerAsupressed:  &R%d\n\r",
	  get_timer( victim, TIMER_ASUPRESSED ) );
    if ( IS_NPC( victim ) )
	ch_printf_color( ch, "&cAct Flags  : &w%s\n\r", ext_flag_string( &victim->act, act_flags ) );

    if (!IS_NPC(victim))
    {
       if ( ch->level >= 65 )
       {
    	ch_printf_color( ch, "&cRage: &w%d \n\r", victim->rage );
        ch_printf_color( ch, "&cPlayerFlags: &w%s\n\r", ext_flag_string( &victim->act, plr_flags ) );
	if (!xIS_EMPTY(victim->powerup))
        ch_printf_color( ch, "&cPUFlags    : &w%s\n\r", ext_flag_string( &victim->powerup, pu_flags ) );
	if (!xIS_EMPTY(victim->skill))
        ch_printf_color( ch, "&cSkillFlags : &w%s\n\r", ext_flag_string( &victim->skill, skill_flags ) );
       }
	ch_printf_color( ch, "&cPcflags    : &w%s\n\r", ext_flag_string( &victim->pcdata->flags, pc_flags ) );
    }
    ch_printf_color( ch, "&cPowerlevel: &C%d\n\r", victim->exp );
    if ( !IS_NPC( victim ) && victim->wait )
	ch_printf_color( ch, "   &cWaitState: &R%d\n\r",
	  victim->wait/12 );
    else
	send_to_char( "\n\r", ch );
    send_to_char( "\n\r", ch );
    if ( victim->pcdata && victim->pcdata->bestowments 
      ...
      ch_printf_color( ch, "&cBestowments: &w%s\n\r", victim->pcdata->bestowments );
    if ( IS_NPC( victim ) )
      ch_printf_color( ch, "&cShortdesc  : &w%s\n\r&cLongdesc   : &w%s",
	...
	if ( victim->long_descr[0] != '\0' )
	  ch_printf_color( ch, "&cLongdesc   : &w%s\n\r",
	    victim->long_descr );
    }
   if( IS_NPC(victim) && victim->spec_fun )
      pager_printf_color( ch, "&cMobile has spec fun: &w%s\n\r", victim->spec_funname );
    if ( IS_NPC( victim ) )
      ch_printf_color( ch, "&cBody Parts : &w%s\n\r",
	flag_string(victim->xflags, part_flags) );
    if ( IS_NPC( victim ) )
    {
      CLAN_DATA *clan;
      ch_printf_color( ch, "&cAttacks    : &w%s\n\r",
	ext_flag_string(&victim->attacks, attack_flags) );
      ch_printf_color( ch, "&cDefenses   : &w%s\n\r",
	ext_flag_string(&victim->defenses, defense_flags) );
      clan = get_clan_short( victim->pIndexData->clan );
      ch_printf_color( ch, "&cClan       :&w %s\n\r", clan ? clan->name : "None" );
    }
	pager_printf_color( ch, "&cMap   : &W%s &w &cCoords: &w%d %d\n\r",
		IS_PLR_FLAG( victim, PLR_ONMAP ) ? map_names[victim->map] : "none",
		victim->x, victim->y );
        pager_printf_color( ch, "&cMap#  : &W%d\n\r", victim->map );
    if (!IS_NPC(victim) && ch->level >= 65)
    {
     ch_printf_color( ch, "&cBarrier   : &w%s\n\r", num_punct(victim->barrier));
     ch_printf_color( ch, "&cCharge    : &w%s\n\r", num_punct(victim->charge));
     ch_printf_color( ch, "&cSuppress  : &w%s\n\r", num_punct(victim->suppress));   
     ch_printf_color( ch, "&cBiomass   : &w%s\n\r", num_punct(victim->biomass));
     ch_printf_color( ch, "&cPropaganda: &w%d\n\r", get_timer(victim, TIMER_PROPAGANDA));
     ch_printf_color( ch, "&cKimod     : &w%f\n\r", victim->kimod);
     ch_printf_color( ch, "&cMod       : &w%f\n\r", victim->mod);
     ch_printf_color( ch, "&cKili      : &w%d\r\n", victim->pcdata->kili);

    }
    if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_ONMAP))
    send_to_char( "Is on map\n\r", ch);
    return;
}
...
void do_gwhere( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char_color( "&wInvalid level range.\n\r", ch );
        return;
    }
    argument = one_argument( argument, arg3 );
    ...
    if ( !pmobs )
    {
        ...
        {
            found = TRUE;
            ch_printf_color( ch, "&c(&C%2d&c) &w%-12.12s   [%-5d - %-19.19s]   &c%-25.25s\n\r",
    ...
    ch_printf_color( ch, "&c%d %s found.\n\r", count, pmobs ? "mobs" : "characters" );
    return;
}
...
void do_gfighting( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !pmobs && !phating && !phunting)
    {
        ...
        {
            found = TRUE;
            ch_printf_color( ch, "&w%-12.12s &C|%2d &wvs &C%2d| &w%-16.16s [%5d]  &c%-20.20s [%5d]\n\r",
    ...
    else if ( !phating && !phunting)
    {
        for ( victim = first_char; victim; victim = victim->next )
        if ( IS_NPC( victim )
        &&   victim->in_room && can_see( ch, victim, FALSE )
	&&   victim->fighting && victim->level >= low && victim->level <= high )
	{
            found = TRUE;
            ch_printf_color( ch, "&w%-12.12s &C|%2d &wvs &C%2d| &w%-16.16s [%5d]  &c%-20.20s [%5d]\n\r",
    ...
    ch_printf_color( ch, "&c%d %s conflicts located.\n\r", count, pmobs ? "mob" : "character" );
    return;
}
...
void do_bodybag( CHAR_DATA *ch, char *argument )
{
        ...
        && (obj->pIndexData->vnum == 11 ) )
	{
	      send_to_char( "\n\r", ch );
	      found = TRUE;
	      ch_printf_color( ch, "&P%s:  %s%-12.12s %s  &PIn:  &w%-22.22s  &P[&w%5d&P]   &PTimer:  %s%2d",
		bag ? "Bagging" : "Corpse",
		bag ? "&Y"      : "&w",
                capitalize( arg1 ),
    ...
    if ( !found )
    {
	send_to_char_color( "&Pno corpse was found.\n\r", ch );
	return;
    }
    send_to_char( "\n\r", ch );
    ...
    if ( owner == NULL )
    {
	ch_printf_color( ch, "&P%s is not currently online.\n\r",
	  capitalize( arg1 ) );
	return;
    }
    return;
...
void do_owhere( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "all" ) )
    {
       for ( obj = first_object; obj; obj = obj->next )
       {
          ch_printf( ch, "\n\rVnum: %d   Count: %d\n\r", obj->pIndexData->vnum, obj->count );
          continue;
       }
       return;
    ...
    if ( arg1[0] != '\0' && !str_prefix(arg1, "nesthunt") )
    {
      ...
      {
	ch_printf(ch, "[%5d] %-28s in object [%5d] %s\n\r",
	...
	++icnt;
      ...
      send_to_char(buf, ch);
      ++icnt;
      ch_printf(ch, "Nested %d levels deep.\n\r", icnt);
      return;
    }
...
void do_snoop( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "You can't snoop while being fused.\n\r", ch );
        return;
    }

    if ( victim->desc->snoop_by )
    {
	send_to_char( "Busy already. (Someone already snooping)\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Busy already. (Minimum Snoop Level)\n\r", ch );
	return;
    }
...
void do_statshield( CHAR_DATA *ch, char *argument )
{
    ...
    one_argument( argument, arg );
    if ( IS_NPC ( ch ) || (get_trust( ch ) < LEVEL_GREATER && !ch->plr_home)  )
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
//    sh_int vnum;
    ...
    if ( (vnum == 298 || vnum == 297) && ch->level < 65)
    {
        send_to_char( "You cant minvoke Shenlong.\n\r", ch );
        return;
    }
...
//    sh_int vnum;
    ...
    if ( ((vnum >= 253 && vnum <= 266) || (vnum >= 270 && vnum <= 276)) && ch->level < 65)
    {
        send_to_char( "You can't invoke DragonBalls.\n\r", ch );
        return;
    }
    if ( (vnum >= 292 && vnum <= 297)  && ch->level < 65)
    {
        send_to_char( "You can't invoke Legendary Weapons.\n\r", ch );
        return;
    }
...
void do_strip( CHAR_DATA *ch, char *argument )
{
    ...
    for ( obj_lose=victim->first_carrying; obj_lose; obj_lose=obj_next ) {
      ...
      ch_printf_color( ch, "  &G... %s (&g%s) &Gtaken.\n\r",
	capitalize(obj_lose->short_descr), obj_lose->name );
      count++;
    }
    if ( !count )
      ch_printf_color( ch, "&GNothing found to take.\n\r", ch );
    return;
}     
...
void do_users( CHAR_DATA *ch, char *argument )
{
	...
	||   (d->character && can_see( ch, d->character, TRUE )) )
	{
      ...
      {
	if ( (get_trust(ch) >= LEVEL_SUPREME
	||   (d->character && can_see( ch, d->character, FALSE )) )
...
void do_cmdtable( CHAR_DATA *ch, char *argument )
{
    ...
    { 
		...
		{
		    if ((++cnt)%4)
			ch_printf(ch,"%-6.6s %4d\t",cmd->name,cmd->userec.num_uses);
		    else
			ch_printf(ch,"%-6.6s %4d\n\r", cmd->name,cmd->userec.num_uses );
		}
	send_to_char( "\n\r", ch );
    }
    ...
    {
    	set_char_color(AT_IMMORT, ch);
    	send_to_char("Commands that have caused lag this run\n\r", ch);
    	set_char_color(AT_PLAIN, ch);
    	for(cnt = hash = 0; hash < 126; hash++)
    		for(cmd = command_hash[hash]; cmd; cmd = cmd->next)
    		{
    			if(!cmd->lag_count)
    				...
    				ch_printf(ch,"%-6.6s %4d\t",cmd->name,cmd->lag_count);
		    	else
...
void do_form_password( CHAR_DATA *ch, char *argument) 
{
   ...
   if( !arg || arg[0] == '\0' )
   {
	send_to_char( "Usage: formpass <Name> <password>\n\r", ch );
	send_to_char( "Name should be capitalized for proper results.\n\r", ch );
	return;
   }
	...
	else /* just for every room with the appropriate people in it */
	{
		for (i = 0; i < MAX_KEY_HASH; i++) /* run through all the buckets */
			for (room = room_index_hash[i] ; room ; room = room->next)
			{
				...
				if (found && !room_is_private(ch, room) && !IN_HOUSE(ch)) /* Any of the required type here AND room not private? */
				{
...
void do_cset( CHAR_DATA *ch, char *argument )
{
  ...
  { 
    ch_printf_color(ch, "\n\r&WMud_name: %s\n\r", sysdata.mud_name );
    ch_printf(ch, "Emergancy Copyover: %s\n\r", sysdata.ECOPYOVER ? "On" : "Off" );
    ch_printf(ch, "PL Loss: %s\n\r", sysdata.plloss ? "On" : "Off" );
    ...
    ch_printf_color(ch, "  &wTake all mail: &W%d\n\r",
    ...
    ch_printf_color(ch, "&WChannels:\n\r  &wMuse: &W%d   &wThink: &W%d   &wLog: &W%d   &wBuild: &W%d\n\r",
 	    sysdata.muse_level, sysdata.think_level, sysdata.log_level, 
	    sysdata.build_level);
    ch_printf_color(ch, "&WBuilding:\n\r  &wPrototype modification: &W%d  &wPlayer msetting: &W%d\n\r",
	    sysdata.level_modify_proto, sysdata.level_mset_player );
    ch_printf_color(ch, "&WBan Data:\n\r  &wBan Site Level: &W%d   &wBan Class Level: &W%d   ",
		sysdata.ban_site_level, sysdata.ban_class_level );
    ch_printf_color(ch, "&wBan Race Level: &W%d\n\r", sysdata.ban_race_level );
    ch_printf_color(ch, "&WDefenses:\n\r  &wDodge_mod: &W%d    &wParry_mod: &W%d    &wTumble_mod: &W%d\n\r",
				sysdata.dodge_mod, sysdata.parry_mod, sysdata.tumble_mod );
    ch_printf_color(ch, "&WOther:\n\r  &wForce on players:             &W%-2d     ", sysdata.level_forcepc);
    ch_printf_color(ch, "&wPrivate room override:         &W%-2d\n\r", sysdata.level_override_private);
    ch_printf_color(ch, "  &wPenalty to bash plr vs. plr:  &W%-7d", sysdata.bash_plr_vs_plr );
    ch_printf_color(ch, "&wPenalty to non-tank bash:      &W%-3d\n\r", sysdata.bash_nontank );
    ch_printf_color(ch, "  &wPenalty to gouge plr vs. plr: &W%-7d", sysdata.gouge_plr_vs_plr );
    ch_printf_color(ch, "&wPenalty to non-tank gouge:     &W%-3d\n\r", sysdata.gouge_nontank );
    ch_printf_color(ch, "  &wPenalty regular stun chance:  &W%-7d", sysdata.stun_regular );
    ch_printf_color(ch, "&wPenalty to stun plr vs. plr:   &W%-3d\n\r", sysdata.stun_plr_vs_plr );
    ch_printf_color(ch, "  &wPercent damage plr vs. plr:   &W%-7d", sysdata.dam_plr_vs_plr );
    ch_printf_color(ch, "&wPercent damage plr vs. mob:    &W%-3d \n\r", sysdata.dam_plr_vs_mob );
    ch_printf_color(ch, "  &wPercent damage mob vs. plr:   &W%-7d", sysdata.dam_mob_vs_plr );
    ch_printf_color(ch, "&wPercent damage mob vs. mob:    &W%-3d\n\r", sysdata.dam_mob_vs_mob );
    ch_printf_color(ch, "  &wGet object without take flag: &W%-7d", sysdata.level_getobjnotake);
    ch_printf_color(ch, "&wAutosave frequency (minutes):  &W%d\n\r", sysdata.save_frequency );
    ch_printf_color(ch, "  &wMax level difference bestow:  &W%-7d", sysdata.bestow_dif );
    ch_printf_color(ch, "&wChecking Imm_host is:          &W%s\n\r", (sysdata.check_imm_host) ? "ON" : "off" );
    ch_printf_color(ch, "&wSaving Pets is:                &W%s\n\r",  (sysdata.save_pets) ? "ON" : "off" );
    ch_printf_color(ch, "  &wPkill looting is:             &W%s\n\r", (sysdata.pk_loot) ? "ON" : "off" );
    ch_printf_color(ch, "  &wSave flags: &W%s\n\r", flag_string( sysdata.save_flags, save_flag ) );
    ch_printf_color(ch, "  &wIdents retries: &W%d\n\r", sysdata.ident_retries);
    return;
  }
  ...
  if ( !str_cmp( arg, "copyover" ) )
  {
      ...
      if ( sysdata.ECOPYOVER )
           send_to_char( "Emergancy copyover system enabled.\n\r", ch );
      else
           send_to_char( "Emergancy copyver system disabled.\n\r", ch );
      return;
  }
  if ( !str_cmp( arg, "plloss" ) )
  {
      sysdata.plloss = !sysdata.plloss;

      if ( sysdata.plloss )
           send_to_char( "Powerlevel Loss system enabled.\n\r", ch );
      else
           send_to_char( "Powerlevel Loss system disabled.\n\r", ch );
      return;
  }
...
void do_silencenew( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  sh_int time;
  ...
  {
    ch_printf(ch, "They are already silenced until %24.24s, by %s.\n\r",
            ctime(&victim->pcdata->silenced_date), victim->pcdata->silenced_by);
    return;
  }
  ...
  if ( h_d )
    ...
    tms->tm_mday += time;
  victim->pcdata->silenced_date = mktime(tms);
  victim->pcdata->silenced_by = STRALLOC(ch->name);
  ch_printf(ch, "%s will be unsilenced at %24.24s.\n\r", victim->name,
          ctime(&victim->pcdata->silenced_date));
  ch_printf(victim, "The immortals are not pleased with your actions.\n\r"
...
void do_unsilencenew( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  ...
  if ( victim != ch )
    send_to_char( "They have been unsilence.\n\r", ch );
  if ( victim->pcdata->silenced_by )
  {
    if( str_cmp(ch->name, victim->pcdata->silenced_by) )
      ch_printf(ch, "(You should probably write a note to %s, explaining the early unsilence.)\n\r",
            victim->pcdata->silenced_by);
    STRFREE(victim->pcdata->silenced_by);
    victim->pcdata->silenced_by = NULL;
  }

  victim->pcdata->silenced_date = 0;
  xREMOVE_BIT(victim->act, PLR_SILENCE);
  save_char_obj(victim);
  return;
}
...
void do_vsearch( CHAR_DATA *ch, char *argument )
{
	...
	if ( in_obj->carried_by != NULL )
	  ch_printf( ch, "[%2d] Level %d %s carried by %s.\n\r", 
		obj_counter,
		obj->level, obj_short(obj),
		PERS( in_obj->carried_by, ch, TRUE ) );
	else           
	  ch_printf( ch, "[%2d] [%-5d] %s in %s.\n\r", obj_counter,
...
void do_cedit( CHAR_DATA *ch, char *argument )
{
	...
	if ( get_trust(ch) > LEVEL_SUB_IMPLEM  )
	{
	    ...
	    send_to_char( "\n\rField being one of:\n\r", ch );
	    send_to_char( "  level position log code flags ooc fuse\n\r", ch );
...
/*	ch_printf( ch, "Command:  %s\n\rLevel:    %d\n\rPosition: %d\n\rLog:      %d\n\rCode:     %s\n\rFlags:  %s\n\r",
	...
	ch_printf( ch, "Command:  %s\n\rLevel:    %d\n\rPosition: %d\n\rLog:      %d\n\rCode:     %s\n\rOOC:      %d\n\rFlags:   %s",
	    command->name, command->level, command->position, command->log,
	    skill_name(command->do_fun), command->ooc , flag_string(command->flags, cmd_flags));*/
	ch_printf( ch, "Command:  %s\n\rLevel:    %d\n\rPosition: %d\n\rLog:      %d\n\rCode:     %s\n\rOOC:      %d\n\rFuse:     %d\n\rFlags:  %s\n\r", command->name, command->level, command->position, command->log, command->fun_name, command->ooc, command->fuse, flag_string(command->flags, cmd_flags) );
    ...
    if ( !str_cmp( arg2, "fuse" ) )
    {
	int fuse = atoi( argument );

	if ( fuse < 0 || fuse > 2 )
	{
		send_to_char( "Improper format. Format: cedit <command> fuse <0 for none/1 for movement/2 for attacks>\n\r", ch );
		return;
	}
	...
	ch_printf( ch, "Priority placement for [%s]:\n\r", command->name );
...
void do_showclass( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "&wMax Skill Adept: &W%-3d             &wThac0 : &W%-5d     &wThac32: &W%d\n\r",
	...
	{
	    set_char_color( AT_LBLUE, ch );
	    ch_printf( ch, "Male: %-30s Female: %s\n\r",
...
void do_showrace( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
        ...
        {
                ++ct;
                ch_printf(ch, "%2d> %-11s",i,race_table[i]->race_name);
                if ( ct%5==0 )
                   send_to_char("\n\r", ch );
        }
    ...
    send_to_char( buf, ch);
    sprintf( buf, "Height: %3d in.\t\tWeight: %4d lbs.\n\r",
...
"---|-------------|----------------------|---------------|------------|----------\n\r");
      for( pproject = first_project; pproject; pproject = pproject->next )
      {
	   ...
	   for ( log = pproject->first_log; log; log=log->next )
		num_logs++;
	   ch_printf(ch, "%2d | %-11s | %-20s | %-13s | %-10s | %3d\n\r", 
	    pcount, 
	    pproject->owner ? pproject->owner : "(None)", 
 	    pproject->name,
...
"---|-------------|----------------------|--------------------------|-----------\n\r");
	...
	ch_printf(ch, "---|----------------------|--------------------------\n\r");
	 ...
	 if (pproject->status && !str_cmp("Done", pproject->status ) )
		continue;
 	 if(!aflag)
	   ch_printf(ch, "%2d | %-11s | %-20s | %-24s | %-10s\n\r", 
	    pcount, 
	    pproject->owner ? pproject->owner : "(None)", 
 	    pproject->name,
	    pproject->date,
      ...
      if(pcount == 0)
         ch_printf(ch, "No projects exist.\n\r");
      else if( aflag && !projects_available )
	 ch_printf(ch, "No projects available.\n\r"); 
      return;
   }
  ...
  if ( arg[0] == '\0' )
  {
   ...
   for ( d = first_descriptor; d; d = d->next)
    ...
    for ( d = first_descriptor; d; d = d->next)
    {
         fMatch = FALSE;
         if ( (d->connected != CON_PLAYING && d->connected != CON_EDITING)
		|| d->character == NULL || !can_see (ch, d->character, FALSE))
                continue;
         for ( hmm = first_ip; hmm; hmm = hmm->next )
	 {
                if ( !str_cmp( hmm->host, d->host) &&
...
void do_pcrename( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( backname,"%s%s",BACKUP_DIR,capitalize( victim->pcdata->filename ) );
...
void do_delete (CHAR_DATA * ch, char *argument)
{
  ...
  if (arg[0] == '\0')
    {
      send_to_char ("\n\rIf you would like to delete this character, the syntax is Delete <PassWord>:\n\r", ch);
      return;
    }
  if ( is_leader(ch) == TRUE )
  {
     send_to_char ("You can't delete if your a leader!\n\rPlease leave a successor!\n\r", ch);
     return;
  }
...
void do_restorefile( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf( fname, "%s%s", BACKUP_DIR, capitalize( arg ) );
  if ( stat( fname, &fst ) != -1 )
  {
  sprintf( buf2, "%s%c/%s", PLAYER_DIR, tolower(arg[0]), capitalize( arg ) );
  sprintf( buf, "%s%s", BACKUP_DIR, capitalize( arg ) );
  rename( buf, buf2 );
...
void do_fakequit( CHAR_DATA *ch, char *argument )
{
    ...
    set_char_color( AT_WHITE, ch );
    send_to_char( "You have quit, how funny\n\rYou left the game, well not completly ", ch );
