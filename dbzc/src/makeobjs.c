...
void make_debris( OBJ_DATA *obj )
{
  ...
  separate_obj( obj );
  debris        = create_object( get_obj_index( OBJ_VNUM_DEBRIS ), 0 );
  debris->timer = number_range( 15, 30 );
  sprintf( buf, debris->short_descr, obj->short_descr );
  STRFREE( debris->short_descr );
  debris->short_descr = STRALLOC( buf );
  sprintf( buf, debris->description, obj->short_descr );
  STRFREE( debris->description );
  debris->description = STRALLOC( buf );
  if ( obj->in_room )
  {
    if ( (ch = obj->in_room->first_person ) != NULL )
    {
      act( AT_OBJECT, "$p is reduced to little more than debris.",
           ch, obj, NULL, TO_ROOM );
      act( AT_OBJECT, "$p is reduced to little more than debris.",
           ch, obj, NULL, TO_CHAR );
    }
    obj_to_room( debris, obj->in_room, NULL);
  }
  extract_obj( obj );
}
/*
...
/*	Using corpse cost to cheat, since corpses not sellable */
	...
	obj_next = obj->next_content;
        if (!IS_OBJ_STAT( obj, ITEM_FOLLOW))
        {
	obj_from_char( obj );
...
/*
void make_head( CHAR_DATA *ch, CHAR_DATA *Killer )
{
char buf[MAX_STRING_LENGTH];
OBJ_DATA *head;
char *name;
...
obj_to_room( head, ch->in_room, ch );
    return;
}
...
/*
 * Make a tail out of a character.
 */
void make_tail( CHAR_DATA *ch  )
{
    char buf[MAX_STRING_LENGTH];
    OBJ_DATA *tail;
    char *name;
	...
	name	       = ch->name;
	tail	       = create_object(get_obj_index(OBJ_VNUM_TAIL), 0);
        tail->timer    = 20;
        tail->value[2] = (int)(tail->timer/8);
	tail->value[4] = ch->level;

    /* Added tail name - make locate easier , other skills */
    sprintf( buf, "tail %s", name );
    STRFREE( tail->name );
    tail->name = STRALLOC( buf );

    sprintf( buf, tail->short_descr, name );
    STRFREE( tail->short_descr );
    tail->short_descr = STRALLOC( buf );

    sprintf( buf, tail->description, name );
    STRFREE( tail->description );
    tail->description = STRALLOC( buf );
    obj_to_room( tail, ch->in_room, ch );
