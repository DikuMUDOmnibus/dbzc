...
show_bans ( CHAR_DATA *ch, int type )
{
  ...
  switch ( type ) {
	case BAN_SITE:
		send_to_char("Banned sites:\n\r", ch);
      		send_to_char("[ #] Warn (Lv) Time                     By              For   Site\n\r",ch);
 send_to_char("---- ---- ---- ------------------------ --------------- ----  ---------------\n\r", ch);
		pban = first_ban;
 		...
 		for ( bnum=1; pban; pban=pban->next, bnum++)
		{
 		...
 		return;
	case BAN_RACE:
		send_to_char("Banned races:\n\r", ch);
      		send_to_char("[ #] Warn (Lv) Time                     By              For   Race\n\r",ch);
		pban = first_ban_race;
		break;
	case BAN_CLASS:
		send_to_char("Banned classes:\n\r", ch);
      		send_to_char("[ #] Warn (Lv) Time                     By              For   Class\n\r",ch);
		pban = first_ban_class;
		break;
	default:
 ...
 set_char_color( AT_PLAIN, ch );
 for ( bnum=1; pban; pban=pban->next, bnum++)
      ch_printf (ch, "[%2d] %-4s (%2d) %-24s %-15s %4d  %s\n\r", bnum,
