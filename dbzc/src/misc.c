...
void do_buyhome( CHAR_DATA *ch, char *argument )
{
     ...
     if ( !IS_SET( room->room_flags2, ROOM_EMPTY_HOME ) )
     {
     ...
     if ( ch->gold < 15000000 )
     {
         send_to_char( "&RThis room costs 15,000,000 gold coins you don't have enough!\n\r&w", ch);
         return;
     }
     ...
     REMOVE_BIT( room->room_flags2, ROOM_EMPTY_HOME );
     SET_BIT( room->room_flags2, ROOM_PLR_HOME );

     fold_area( room->area, room->area->filename, FALSE, FALSE );

     ch->plr_home = room;
     do_save( ch , "" );

     send_to_char("&GYou buy your home for 15,000,000 gold coins.\n\r", ch);
}
...
void do_sellhome( CHAR_DATA *ch, char *argument )
{
     ...
     if ( !IS_SET( room->room_flags2, ROOM_PLR_HOME ) )
     {
     ...
     if ( ch->plr_home1 != NULL )
     {
        ...
        for ( exitnum = 0 ; exitnum < 11 ; exitnum++ )
        {
          ...
          if(xit != NULL)
             if(xit->to_room->vnum != 8000)
               extract_exit(location, xit);
        }
        char_from_room( ch );
        char_to_room( ch, wasinroom );
        command( ch, do_rdelete, "%d", delete->vnum);
     }
     if ( ch->plr_home2 != NULL )
     {
        char_from_room( ch );
        char_to_room( ch, ch->plr_home2 );
        delete = ch->in_room;
        do_cleanroom( ch, "" );
        gold += 250000;
        for ( exitnum = 0 ; exitnum < 11 ; exitnum++ )
     ...
     if ( ch->plr_home4 != NULL )
     {
      ...
      STRFREE( room->name );
      room->name = STRALLOC( "An empty house" );
      gold += 5000000;
      ch->gold += gold;
      ch_printf( ch, "You sell your house for %s gold coins\n\r", num_punct(gold));
      REMOVE_BIT( room->room_flags2, ROOM_PLR_HOME );
      SET_BIT( room->room_flags2, ROOM_EMPTY_HOME );
...
void do_eat( CHAR_DATA *ch, char *argument )
{
	...
	if ( obj->item_type != ITEM_FOOD && obj->item_type != ITEM_PILL &&
	     obj->item_type != ITEM_COOK && obj->item_type != ITEM_SENSU)
	{
    ...
    if ( !IS_NPC(ch) && (!IS_PKILL(ch) || (IS_PKILL(ch) && !xIS_SET(ch->pcdata->flags, PCFLAG_HIGHGAG) ) ) )
	hgflag = FALSE;
    ...
    if ( ch->fighting && number_percent( ) > ((get_curr_dex(ch) * 2 + 50)) && chance == 1 )
    {
	sprintf( buf, "%s",
...
//        ch->hit = URANGE( 0, ch->hit + (ch->max_hit * 0.60), ch->max_hit );
	  ch->hit = ch->max_hit;
    }
    break;
    }
...
void do_apply( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "Syntax: apply <scroll> <item>\n\r", ch );
	return;
    }
    if ( ch->fighting )
    {
   ...
   if ( scroll->item_type != ITEM_SCROLL )
   {
      act( AT_ACTION, "That isn't a scroll...", ch, scroll, NULL, TO_CHAR );
      return;	
   }        
   if ( obj->item_type != ITEM_ARMOR && obj->item_type != ITEM_WEAPON && obj->item_type != ITEM_JEWELRY )
   {
       send_to_char( "You can't do that to that object.\n\r", ch);
       return;
   }
   if ( obj->value[4] > 10 )
   {
       send_to_char( "You can't enchant it any further.\n\r", ch);
       return;
   }
   if ( obj->value[3] != scroll->value[3] )
   {
       send_to_char( "The scroll and the armor must be the same grade.\n\r", ch);
       return;
   }
   if ( obj->value[4] > 3 ) /* Chance for it to fail */
   {
      sh_int chance = number_percent();
       
      if ( (obj->value[4] * number_range( 8, 11 )) > chance )
      {
          ch_printf( ch, "\n\r\n\r&YYou begin to enchant %s&Y onto %s&Y...\n\r", scroll->short_descr, obj->short_descr);
	  ch_printf( ch, "&OAfter finishing to enchant the scroll it dissappears...\n\r\n\r");
	  ch_printf( ch, "&R%s&R begins to deteriorate until reaching non-existence...\n\r\n\r", obj->short_descr);
          sprintf( buf, "\n\r&Y%s begins to enchant %s&Y onto %s&Y...", ch->name, scroll->short_descr, obj->short_descr);
	  act( AT_WHITE, buf, ch, NULL, NULL, TO_ROOM );
          sprintf( buf, "&OAfter finishing to enchant the scroll it dissappears...\n\r");
          act( AT_WHITE, buf, ch, NULL, NULL, TO_ROOM );
          sprintf( buf, "&R%s&R begins to deteriorate until reaching non-existence...\n\r", obj->short_descr);
          act( AT_WHITE, buf, ch, NULL, NULL, TO_ROOM );
          separate_obj( obj );
          separate_obj( scroll );
          extract_obj( scroll );
    ...
    ch_printf( ch, "\n\r\n\r&YYou begin to enchant %s&Y onto %s&Y...\n\r", scroll->short_descr, obj->short_descr);
    ch_printf( ch, "&OAfter finishing to enchant the scroll it dissappears...\n\r\n\r");
     ch_printf( ch, "&G%s&G begins to glow brightly...\n\r&GYou have successfully enchanted it to &Y+%d\n\r\n\r", obj->short_descr, obj->value[4]);
    sprintf( buf, "\n\r&Y%s begins to enchant %s&Y onto %s&Y...", ch->name, scroll->short_descr, obj->short_descr);
    act( AT_WHITE, buf, ch, NULL, NULL, TO_ROOM );
    sprintf( buf, "&OAfter finishing to enchant the scroll it dissappears...\n\r");
    act( AT_WHITE, buf, ch, NULL, NULL, TO_ROOM );
    sprintf( buf, "&G%s&G begins to glow brightly...\n\r&G%s has successfully enchanted it to &Y+%d\n\r", obj->short_descr, ch->name, obj->value[4]);
    act( AT_WHITE, buf, ch, NULL, NULL, TO_ROOM );
...
//     save_classes();
    ...
    for ( barea = first_build; barea; barea = barea->next )
    {
      ...
      fold_area( barea, filename, FALSE, FALSE );
    }
   send_to_char( "Saved.\n\r", ch);
}
...
void do_kickoff( CHAR_DATA *ch, char *argument )
{
    ...
    {
      sprintf( buf, "&w&G%s takes their baseball bat and hits %s off the MUD!", ch->name,victim->name);
      do_echo(ch, buf);
    }
    else if ( !str_cmp( arg2, "bungee" ) )
    {
      sprintf( buf, "&w&G%s ties %s to a bungee cord and throws them off the MUD!",ch->name, victim->name);
      do_echo(ch, buf);
    }
    else if ( !str_cmp( arg2, "gun" ) )
    {
       sprintf( buf, "&w&G%s takes a gun and shoots %s and watchs their lifeless body fall off the MUD!", ch->name,victim->name);
      do_echo(ch, buf);
    }
    ...
    {
      sprintf( buf, "&w&G%s bitchslaps %s so hard, they are kicked off the MUD!",ch->name,victim->name);
      do_echo(ch, buf);
    }
...
void do_homename( CHAR_DATA *ch, char *argument )
{
     ROOM_INDEX_DATA *room;
     AREA_DATA *pArea;
     ...
     if ( ch->plr_home == NULL )
     {
         send_to_char( "&RYou dont even HAVE a home!\n\r&w", ch);
         return;
     }

     if (ch->in_room != ch->plr_home
     &&  ch->in_room != ch->plr_home1
     &&  ch->in_room != ch->plr_home2
     &&  ch->in_room != ch->plr_home3
     &&  ch->in_room != ch->plr_home4  )
     {
         send_to_char( "&RYou must be in your home to rename it!\n\r&w", ch);
         return;
     }

     if ( ch->gold < 100 )
     {
         send_to_char( "&RIt costs 100 gold coins to rename a home! You don't have enough!\n\r&w", ch);
         return;
     }
     ...
     {
           send_to_char( "Set the room name.  A very brief single line room description.\n\r", ch );
           send_to_char( "Usage: Namehome <Room Name>\n\r", ch );
           return;
     }
     ...
     send_to_char("You pay 10000 gold coins to rename your home.\n\r", ch);
...
void do_homedesc( CHAR_DATA *ch, char *argument )
{
   ROOM_INDEX_DATA *room;
   ROOM_INDEX_DATA *location;
   ...
   if ( ch->gold < 50000 )
   {
     send_to_char( "You do not have 50,000 gold goins. \n\r", ch);
     return;
   }

   if ( IS_SET(ch->in_room->room_flags2, ROOM_EMPTY_HOME) )
   {
   ...
   &&  ch->in_room != ch->plr_home4  )
   {
         send_to_char( "&RYou must be in your home to re-description it!\n\r&w", ch);
         return;
   }
...
void do_invite( CHAR_DATA *ch, char *argument )
{
	...
	ENTRANCE_DATA *entrance;
        ...
        if ( arg[0] == '\0' )
        {
                send_to_char( "Who do you want to invite into your home?\n\r", ch );
                return;
        }

        if ( ( victim = get_char_world( ch, arg )) == NULL)
        {
                send_to_char("YYou dont see anyone!\n\r",ch);
                return;
        }
        if ( ch->in_room != ch->plr_home )
            return;
		...
		return;
           ...
           entrance = checkforentrance( ch->in_room->vnum );
           if ( !entrance )
 	   {
                send_to_char( "You must be standing inside the entrance of your house", ch );
		return;
           }
           if ( victim->buzzedx != victim->x || victim->buzzedy != victim->y || victim->buzzedmap != victim->map )
           {
		send_to_char( "They must be standing outside.\n\r",ch);
		return;
	   }
	   if ( entrance->fromx != victim->buzzedx || entrance->fromy != victim->buzzedy || entrance->onmap != victim->buzzedmap )
           {
                send_to_char( "They must be standing outside.\n\r",ch);
                return;
           }
           act( AT_ACTION, "$N gets invited into $n's room!", ch, NULL, victim, TO_ROOM);
           act( AT_ACTION, "$n gets invited into $N's room!", victim, NULL, ch, TO_ROOM);
	   ch_printf( victim, "%s invites you in!\n\r", ch->name );
           ch_printf( ch, "You invite %s in!\n\r", victim->name );
	   xREMOVE_BIT(victim->act, PLR_ONMAP);
	   victim->x = victim->y = victim->map = victim->buzzedx = victim->buzzedy = victim->buzzedmap = -1;
	   char_from_room( victim );
	   char_to_room( victim, ch->in_room );
           return;
        }
        if ( !victim->buzzedfrom )
                return;
        if ( victim->buzzed != ch->in_room->vnum || victim->buzzedfrom != victim->in_room->vnum )
        {
        ...
        char_from_room( victim );
        char_to_room( victim , ch->in_room );
        act( AT_ACTION, "$N gets invited into $n's room!", ch, NULL, victim, TO_NOTVICT);
        return;
}
...
void do_buzz( CHAR_DATA *ch, char *argument )
{
	...
	ROOM_INDEX_DATA *room;
        ...
        sh_int	   direction, newx, newy;
        direction = newx = newy = 0;
        one_argument( argument, arg );

        if ( arg[0] == '\0' )
        {
                send_to_char( "Buzz in which direction?\n\r", ch );
                return;
        }
        if ( ch->fighting )
        {
                send_to_char( "Your a little busy right about now.\n\r", ch );
                return;
        }
        if ( IS_PLR_FLAG( ch, PLR_ONMAP) )
        {
		  ...
		  entrance = check_entrance( ch->map, newx, newy );
		  if ( !entrance )
		  {
			send_to_char( "There seems to be no entrance in that direction.\n\r", ch);
			return;
		  }
		  room = get_room_index( entrance->vnum );
		  if ( !room )
		  {
			send_to_char( "Something went wrong, contact an immortal.\n\r", ch);
			return;
		  }
		  if ( entrance->fromx != ch->x && entrance->fromy != ch->y )
		  {
			send_to_char( "You must be facing the entrance.\n\r", ch);
			return;
 		  }
		  if ( entrance->exit_symbol != EXIT_HOUSE )
                  {
                        send_to_char( "That isn't a house.\n\r", ch);
                        return;
                  }
		  if ( !IS_SET(room->room_flags2, ROOM_PLR_HOME) )
                  {
                        send_to_char( "That isn't a player's home.\n\r", ch);
                        return;
                  }
		  ch->buzzedx = ch->x;
		  ch->buzzedy = ch->y;
		  ch->buzzedmap = ch->map;
		  ch->buzzwasonmap = TRUE;
		  ch->buzzed = room->vnum;
		
		  sprintf( buf, "%s uses the intercom from the outside!\n\r", ch->name);
		  echo_to_room( AT_WHITE, room, buf );
                  act( AT_ACTION, "You use the intercom to the $d.", ch, NULL, dir_name[direction], TO_CHAR );
                  act( AT_ACTION, "$n uses the intercom to the $d.", ch, NULL, dir_name[direction], TO_ROOM );
		  return;
        ...
        if ( ( pexit = get_exit(ch->in_room,get_dir(arg) )) != NULL)
        {
                ...
                if ( (to_room = pexit->to_room) != NULL && IS_SET( to_room->room_flags2, ROOM_EMPTY_HOME ) )
                {
                        send_to_char( "You dont need to use the intercom, nobody lives there!\n\r", ch);
                        return;
                }
                else if ( (to_room = pexit->to_room) != NULL
                        && !IS_SET(to_room->room_flags2, ROOM_PLR_HOME) )
                {
...
void do_oproto(CHAR_DATA *ch, char *argument)
{
   command( ch, do_oset, "%s flags prototype", argument);
}
void do_mproto(CHAR_DATA *ch, char *argument)
{
   command( ch, do_mset, "%s flags prototype", argument);
}
void create_room( CHAR_DATA *ch, char *argument, int vnum )
{
  int tmp;
   ...
   if ( ch->level < 55)
   {
    tmp = ch->level;
    ch->level = 55;
    command( ch, do_redit, "bexit %s %d", argument, vnum);
    ch->level = tmp;
   }
   else
    command( ch, do_redit, "bexit %s %d", argument, vnum);
}
void do_homeroom( CHAR_DATA *ch, char *argument )
{
    ROOM_INDEX_DATA     *location;
    ROOM_INDEX_DATA     *location2;
    ROOM_INDEX_DATA     *wasinroom;
    ROOM_INDEX_DATA     *area;
    EXIT_DATA           *xit;
    AREA_DATA           *pArea;
    int                  vnum;
    int                  room_count;
    ...
    x = 1;
     ...
     &&  ch->in_room != ch->plr_home4  )
     {
         send_to_char( "&RYou must be in your home to create more rooms!\n\r&w", ch);
         return;
     }
     ...
     if ( ch->gold < 5000000 )
     {
         send_to_char( "&RIt costs 5000000 gold coins to create a room! You don't have enough!\n\r&w", ch);
         return;
     }
    ...
    {
        send_to_char( "Usage: HomeRoom <Direction>\n\r", ch );
        return;
    }
    if ( ch->plr_home1 && ch->plr_home2 && ch->plr_home3 && ch->plr_home4 )
    {
        send_to_char( "&RYou can't create any more rooms.\n\r", ch );
        return;
    }
    room_count = 0;
    ...
    while(room_count < x)
    {
      ...
      if(get_room_index(vnum) == NULL)
      {
        ...
        location = ch->in_room;
	if ( !ch->plr_home1 )
	  ch->plr_home1 = location;
	else if ( !ch->plr_home2 )
	 ch->plr_home2 = location;
	else if ( !ch->plr_home3 )
	  ch->plr_home3 = location;
	else if ( !ch->plr_home4 )
	  ch->plr_home4 = location;
        SET_BIT( location->room_flags2, ROOM_HOUSE );
    ...
    vnum -= 1;
    wasinroom = ch->in_room;
    ch_printf( ch, "&RNew HomeRoom Created.\r\n", room_count);
...
void do_homeflag( CHAR_DATA *ch, char *argument)
{
     ROOM_INDEX_DATA *room;
     AREA_DATA *pArea;
     ...
     if ( ch->gold < 10000000 )
     {
       send_to_char( "You need 10,000,000 coins to do that.\n\r", ch);
       return;
     }
     room = ch->in_room;
     for ( pArea = first_bsort; pArea; pArea = pArea->next_sort )
     ...
     {
       if (!IS_SET(ch->in_room->room_flags2, ROOM_HEAL))
       {
         ch->gold -= 10000000;
         SET_BIT(ch->in_room->room_flags2, ROOM_HEAL);
         send_to_char("&GCreating Healing Room\n\r", ch);
       }
       else
       send_to_char( "This room already is a healing room.\n\r", ch);
     }
     else if ( !str_cmp( argument, "gravity" ) )
     {
       if (!IS_SET(ch->in_room->room_flags2, ROOM_GRAVITY))
       {
         ch->gold -= 10000000;
         SET_BIT(ch->in_room->room_flags2, ROOM_GRAVITY);
         send_to_char("&GCreating Gravity Room\n\r", ch);
       }
       else
       send_to_char( "This room already is a gravity room.\n\r", ch);
     }
/*     else if ( !str_cmp( argument, "spar" ) )
     {
       if (!IS_SET(ch->in_room->room_flags2, ROOM_SPAR))
       {
         ch->gold -= 10000000;
         SET_BIT(ch->in_room->room_flags2, ROOM_SPAR);
         send_to_char("&GCreating Sparring Room\n\r", ch);
       }
       else
       send_to_char( "This room already is a sparring room\n\r", ch);
     }*/
     else
     {
        send_to_char( "Syntax: HomeFlag [Heal|Gravity]\n\r", ch);
        return;
     }
