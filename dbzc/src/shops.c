...
void do_buy( CHAR_DATA *ch, char *argument )
{
		...
		+ (noi > 1 ? 2 : 0) > can_carry_w( ch ) )
	{
        ...
        {
          send_to_char("You can't buy any more beans!", ch);
          return;
        }
	...
	if ( noi == 1 )
	{
        ...
        else if ( obj->item_type == ITEM_SENSU )
        {
          send_to_char("You can only buy 1 bean at a time", ch);
          return;
        }
/*
        else
...
/*      bool listall; */
                ...
                obj = obj->next_content )
        {
            ...
            && ( arg[0] == '\0' || nifty_is_name( arg, obj->name ) ) )
            {
                ...
                ch_printf( ch, "&z|  [&w%s&z]  &z|  &C%10d &z| &w%s.\n\r",
                    get_grade(obj->pIndexData->value[3]), cost,  obj->short_descr  );
            }
        }
