...
void infochan2(char *argument)
{
  char buf1[MAX_INPUT_LENGTH];
  DESCRIPTOR_DATA *i;

  sprintf(buf1, "&b|&RINFO&b|&z: &Y%s&w\r\n", argument);

  for (i = first_descriptor; i; i = i->next)
  {
    if (!i->connected && i->character)
    {
      if(IS_SET( i->character->deaf, CHANNEL_INFO2))
        continue;
...
void adjust_hiscore( char *keyword, CHAR_DATA *ch, int score, bool silent );
*/
...
/* Hiscore command */
{
	...
	{
		some_table = FALSE;
		send_to_char( "&YHiscore tables in this game&O:\n\r", ch );
	 	send_to_char( "&z-----------------------------------------------------------\n\r\n\r", ch);
		for( table = first_table ; table ; table = table->next )
		{
			if( table->vnum == -1 )
			{
				ch_printf( ch, "  &R%-10.10s&r:  &z'&Y%s&z'\n\r", capitalize(table->keyword), table->desc );
				some_table = TRUE;
			}
			else
			{
 ...
 * way. */
{
	...
	for( num=0; num<len ; num++ )
		buf[num] = ' ';
	buf[num] = '\0';
	pager_printf_color( ch, "&z||     &R%s%s%s%s     &z||&w\n\r",
		buf, table->desc, 
		odd ? " " : "", buf );
	...
	{
		pager_printf_color( ch, "&z||     &Y%2d&z)%s %-20.20s&W%15d     &z||\n\r",
			num, num <= 3 ? "&C" : "&c",
			capitalize(entry->name), entry->score );
		num++;
	}
 ...
 * if the player entered the table */
{
	...
	if( pos != -1 && pos != old_pos && table != NULL )
	{
		...
		sprintf( buf, "&C%s &chas reached position &C%d&c in the table of &z'&C%s&z'&w",
