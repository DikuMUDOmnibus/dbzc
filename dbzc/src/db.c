...
sh_int		gsn_pbarrier;
sh_int		gsn_kyodaika;
sh_int		gsn_timefreeze;
sh_int		gsn_soulcry;

sh_int  		gsn_donut;
sh_int  		gsn_dbeam;
sh_int  		gsn_mouthblast;
sh_int  		gsn_fkame;
sh_int			gsn_chikone;
sh_int                  gsn_chiktwo;
sh_int                  gsn_chikthree;
sh_int                  gsn_chikfour;
sh_int			gsn_chikfive;
sh_int                  gsn_primary;
sh_int			gsn_secondary;
sh_int			gsn_primaryatt;
sh_int                  gsn_energyball;
sh_int                  gsn_sphere;
sh_int			gsn_regen;
sh_int			gsn_heal;
sh_int                  gsn_grip;
sh_int			gsn_absorb;
sh_int			gsn_absorption;
sh_int			gsn_suppress;
sh_int			gsn_ebar;
sh_int			gsn_sbc;
sh_int			gsn_kame;
...
sh_int			gsn_fbeam;
sh_int                  gsn_galic;
sh_int			gsn_ghostattack;
sh_int			gsn_dragonfist;
sh_int			gsn_deathball;
sh_int			gsn_dodompa;
sh_int			gsn_makosen;
sh_int			gsn_burnattack;
sh_int			gsn_renzoku;
sh_int			gsn_finalflash;
sh_int			gsn_bbk;
sh_int			gsn_hellsflash;
sh_int			gsn_rocketpunch;
sh_int                  gsn_disk;
sh_int                  gsn_ddd;
sh_int                  gsn_mystic;
sh_int                  gsn_fly;
sh_int                  gsn_truesense;
sh_int			gsn_kick;
sh_int                  gsn_direct;
...
/* for searching */
sh_int                  gsn_first_askill;
sh_int                  gsn_first_sskill;
...
void    load_income	args( ( AREA_DATA *tarea, FILE *fp ) );
void    load_popsupport args( ( AREA_DATA *tarea, FILE *fp ) );
void    load_government args( ( AREA_DATA *tarea, FILE *fp ) );
void    load_guards     args( ( AREA_DATA *tarea, FILE *fp ) );
...
void    load_ships      args( ( AREA_DATA *tarea, FILE *fp ) );
...
void boot_db( bool fCopyOver )
{
   ...
   load_specfuns();
    ...
    for ( x = 0; x < top_sskill; x++ )
        if ( !gsn_first_sskill && sskill_table[x]->type == SKILL_SSKILL )
            gsn_first_sskill = x;
    for ( x = 0; x < top_askill; x++ )
        if ( !gsn_first_askill && askill_table[x]->type == SKILL_ASKILL )
            gsn_first_askill = x;
    ...
    log_string("Loading Skills  Table");
    load_sskill_table();
    log_string("Loading Abilities Table");
    load_askill_table();
    log_string("Loading Docks" );
    load_docks();
	...
	*/
	...
	log_string("Assigning gsn's");
        ASSIGN_GSN( gsn_charge,         "kicharge" );
	ASSIGN_GSN( gsn_accuracy,   	"accuracy" );
	ASSIGN_GSN( gsn_zanzoken,   	"zanzoken" );
	ASSIGN_GSN( gsn_vstance,   	"vicious stance" );
	ASSIGN_GSN( gsn_relax,   	"relax" );
	ASSIGN_AGSN( gsn_enrage,   	"enrage" );
	ASSIGN_AGSN( gsn_berserk,   	"berserk" );
	ASSIGN_AGSN( gsn_overdrive, 	"overdrive" );
	ASSIGN_AGSN( gsn_bbarrier,   	"blue barrier" );
	ASSIGN_AGSN( gsn_pbarrier,   	"pink barrier" );
	ASSIGN_AGSN( gsn_kyodaika,   	"kyodaika" );
	ASSIGN_AGSN( gsn_timefreeze, 	"timefreeze" );
        ASSIGN_AGSN( gsn_soulcry,    "soulcry" );

	ASSIGN_GSN( gsn_fuse,		"fusion" );
        ASSIGN_GSN( gsn_chikone,          "chikara 1" );
        ASSIGN_AGSN( gsn_donut,          "galactic donut" );
        ASSIGN_AGSN( gsn_dbeam,          "deathbeam" );
        ASSIGN_GSN( gsn_chiktwo,          "chikara 2" );
        ASSIGN_GSN( gsn_chikthree,          "chikara 3" );
        ASSIGN_GSN( gsn_chikfour,          "chikara 4" );
	ASSIGN_GSN( gsn_chikfive,	   "chikara 5" );
        ASSIGN_GSN( gsn_energyball,     "energy ball" );
        ASSIGN_AGSN( gsn_primary,        "primary skill" );
	ASSIGN_AGSN( gsn_secondary,	"secondary skill" );
        ASSIGN_AGSN( gsn_primaryatt,     "primary attack");
	ASSIGN_AGSN( gsn_regen,		"regenerate");
        ASSIGN_AGSN( gsn_sphere,         "energy sphere");
	ASSIGN_AGSN( gsn_heal,		"heal");
	ASSIGN_AGSN( gsn_grip,		"grip");
	ASSIGN_AGSN( gsn_absorb,		"absorb");
	ASSIGN_AGSN( gsn_absorption,	"absorption");
	ASSIGN_GSN( gsn_suppress,       "suppress");
	ASSIGN_AGSN( gsn_ebar,		"energy barrier");
	ASSIGN_AGSN( gsn_kame,		"kamehameha");
	ASSIGN_AGSN( gsn_sbc,		"special beam cannon");
	ASSIGN_GSN( gsn_be,		"Battle Experience");
	ASSIGN_GSN( gsn_blast, 		"blast");
	ASSIGN_AGSN( gsn_masenko,	"masenko");
	ASSIGN_AGSN( gsn_tribeam, 	"tribeam");
	ASSIGN_AGSN( gsn_scattershot,    "scattershot");
	ASSIGN_AGSN( gsn_mental,		"mentalthrow" );
   	ASSIGN_AGSN( gsn_kaioken,  	"kaioken" );
	ASSIGN_AGSN( gsn_dk,             "doublekick");
	ASSIGN_AGSN( gsn_dp,		"doublepunch");
	ASSIGN_AGSN( gsn_eyebeam,        "eyebeam");
	ASSIGN_AGSN( gsn_claw,		"claw");
	ASSIGN_AGSN( gsn_fbeam,		"fingerbeam");
        ASSIGN_AGSN( gsn_galic,          "galic gun" );
	ASSIGN_AGSN( gsn_hellsflash,	"hells flash" );
	ASSIGN_AGSN( gsn_rocketpunch,    "rocketpunch" );
        ASSIGN_AGSN( gsn_finalflash,     "final flash" );
        ASSIGN_AGSN( gsn_fkame,     "final kamehameha" );
        ASSIGN_AGSN( gsn_bbk,     "big bang kamehameha" );
        ASSIGN_AGSN( gsn_dragonfist,     "dragonfist" );
	ASSIGN_AGSN( gsn_ghostattack,	"super ghost kamikaze attack" );
	ASSIGN_AGSN( gsn_deathball,	"deathball");
	ASSIGN_AGSN( gsn_dodompa,	"dodompa" );
	ASSIGN_AGSN( gsn_makosen, 	"makosen" );
	ASSIGN_AGSN( gsn_burnattack,	"burning attack" );
	ASSIGN_AGSN( gsn_renzoku, 	"renzoku energy dan" );
        ASSIGN_AGSN( gsn_disk,           "destructo disk" );
        ASSIGN_AGSN( gsn_ddd,            "dual destructo disk" );
        ASSIGN_AGSN( gsn_mystic,         "mystic" );
        ASSIGN_GSN( gsn_fly,            "fly" );
        ASSIGN_GSN( gsn_truesense,      "truesense" );
	ASSIGN_GSN( gsn_kick,		"kick" );
        ASSIGN_GSN( gsn_direct,         "direct" );
	ASSIGN_GSN( gsn_second_attack, 	"second attack" );
	ASSIGN_GSN( gsn_punch,		"punch" );
	ASSIGN_GSN( gsn_aid,		"aid" );
	ASSIGN_AGSN( gsn_tail,		"tail" );
      ASSIGN_AGSN( gsn_mouthblast,  "mouthblast" );
  }
...
void load_guards( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
        bug( "Load_guards: no #AREA seen yet." );
        if ( fBootDb )
        {
          shutdown_mud( "No #AREA" );
          exit( 1 );
    ...
    tarea->guard1 = fread_number( fp );
    tarea->guard2 = fread_number( fp );
    tarea->guard3 = fread_number( fp );
    tarea->guard4  = fread_number( fp );
    return;
}
...
void load_government( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
        bug( "Load_government: no #AREA seen yet." );
        if ( fBootDb )
        {
          shutdown_mud( "No #AREA" );
          exit( 1 );
    ...
    tarea->governed_by = fread_string( fp );
    return;
}

void load_income( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
        bug( "Load_Income: no #AREA seen yet." );
        if ( fBootDb )
        {
          shutdown_mud( "No #AREA" );
          exit( 1 );
    ...
    tarea->income = fread_number( fp );
    return;
}
void load_popsupport( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
        bug( "Load_Popsupport: no #AREA seen yet." );
        if ( fBootDb )
        {
          shutdown_mud( "No #AREA" );
          exit( 1 );
    ...
    tarea->popsupport = fread_number( fp );
    return;
}
...
void load_objects( AREA_DATA *tarea, FILE *fp )
{
	...
	pObjIndex->rent		  	= fread_number( fp ); /* unused */
        pObjIndex->pShip		= NULL;
/*        pObjIndex->shiproom             = fread_number( fp );
        pObjIndex->landing              = fread_number( fp );
        pObjIndex->departure            = fread_number( fp );*/
        if( area_version >= 1 )
	{
...
void load_ships( AREA_DATA *tarea, FILE *fp )
{
    SHIP_DATA *pShip;
    ...
    for ( ; ; )
    {
        ...
        CREATE( pShip, SHIP_DATA, 1 );
        pShip->obj_vnum          = fread_number( fp );
        if ( pShip->obj_vnum == 0 )
        {
            DISPOSE( pShip );
            break;
        }
        pShip->cockpit       = fread_number( fp );
        pShip->home          = fread_number( fp );
        pShip->speed         = fread_number( fp );
        pShip->current_loc   = fread_number( fp );
        pShip->keyvnum	     = fread_number( fp );
        pShip->entrance      = fread_number( fp );
        pShip->destination   = fread_number( fp );
        pShip->lock	     = fread_number( fp );
        pShip->x1	     = fread_number( fp );
        pShip->y1	     = fread_number( fp );
        pShip->z1	     = fread_number( fp );
        pShip->d1	     = fread_number( fp );
        pShip->x2	     = fread_number( fp );
        pShip->y2	     = fread_number( fp );
        pShip->z2	     = fread_number( fp );
        pShip->d2	     = fread_number( fp );
        pShip->ship_state    = fread_number( fp );
        pShip->name          = fread_string( fp );
        if (pShip->lock == SHIP_SHUTTLE)
        {
          pShip->first_stop  = get_dock( fread_string( fp ) );
          pShip->second_stop = get_dock( fread_string( fp ) );
	  pShip->launch1     = fread_number( fp );
	  pShip->launch2     = fread_number( fp );
        }
        pObjIndex               = get_obj_index( pShip->obj_vnum );
        pObjIndex->pShip        = pShip;

        if ( !first_ship )
            first_ship          = pShip;
        else
            last_ship->next     = pShip;
        pShip->next             = NULL;
        pShip->prev             = last_ship;
        last_ship               = pShip;
        top_ship++;
    }
    return;
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
     ...
     */
/*    if ( pMobIndex->ac )
	mob->armory		= pMobIndex->ac;
    else
	mob->armory		= interpolate( mob->level, 100, -100 );
*/
    mob->max_hit                = 150;
    mob->hit			= mob->max_hit;
...
{ 
    ...
    do
    {
        if ( feof(fp) )
        {
          bug("float_number: EOF encountered on read.\n\r");
          if ( fBootDb )
            exit(1);
          return 0;
        }
    ...
    if ( !isdigit(c) )
    {
        bug( "Float_number: bad format. (%c)", c );
        if ( fBootDb )
          exit( 1 );
        return 0;
    }
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "&w\t\t\t&wAreas:   &W%5d\n\r",
        top_area   );
    ...
    ch_printf_color( ch, "&wMaxEver: &W%5d\t\t\t&wTopsn:   &W%5d(%d)\n\r",
 	sysdata.alltimemax, top_sskill, MAX_SKILL );
...
void towizfile( const char *line, int Border )
{
  ...
  if(Border >0)
  {
      ...
      if(SNIP)
        filler++;
	 if (Border == 1)
      sprintf(outline2,"         &z||&W%s&z",outline);
         if (Border == 2)
      sprintf(outline2,"         &z||&Y%s&z",outline);
...
void make_wizlist( )
{
  ...
  sprintf(buf,"\n\r         &z++==============================================================++&w");
  towizfile(buf, 0);
  sprintf(buf,"&z         ||             &WS&ztaff &WO&zf &YDragon Ball Z: Chronicles&z               ||&w");
  towizfile(buf, 0);
/*  towizfile( " Masters of the Realms of Despair!" );*/
  ...
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
	...
	buf[0] = '\0';
      ...
      towizfile( "         &z||                                                              ||&w",0 );
      ilevel = wiz->level;
      if ( ilevel == LEVEL_SUPREME2)
      ilevel = LEVEL_SUPREME;
      switch(ilevel)
      {
        case MAX_LEVEL -  0:
	case MAX_LEVEL -  1: towizfile( "  MUD Owners", 2 );	break;
	case MAX_LEVEL -  2: towizfile( "  MUD Co-Owners", 2 );	break;
	case MAX_LEVEL -  3: towizfile( "  Supervisors", 2 );	break;
	case MAX_LEVEL -  4: towizfile( "  Head Builders", 2 );	break;
	case MAX_LEVEL -  5: towizfile( "  Head Roleplay", 2 );	break;
	case MAX_LEVEL -  6: towizfile( "  Head Enforcers", 2 );	break;
	case MAX_LEVEL -  7: towizfile( "  Ancient Builders", 2 );	break;
	case MAX_LEVEL -  8: towizfile( "  Greater Builders", 2 );	break;
	case MAX_LEVEL -  9: towizfile( "  Lesser Builders", 2 );	break;
	case MAX_LEVEL - 10: towizfile( "  Clan Managers", 2 );	break;
	case MAX_LEVEL - 11: towizfile( "  RP Specialists", 2 );	break;
	case MAX_LEVEL - 12: towizfile( "  Senior Builders", 2 );	break;
	case MAX_LEVEL - 13: towizfile( "  Junior Builders", 2 );	break;
	case MAX_LEVEL - 14: towizfile( "  Trainee Builders", 2 );  break;
	case MAX_LEVEL - 15: towizfile( "  Player Testers", 2 );   	break;
	case MAX_LEVEL - 16: towizfile( "  RP Assistants", 2 );	break;
	default:	     towizfile( "  Retired", 2 );		break;
...
/*
    ...
    strcat(buf2, "&z||");
    strcat(buf, buf2);
    if ( strlen( buf ) + strlen( wiz->name ) > 80 )
    {
        towizfile( buf );
        buf[0] = '\0';
    }
...
int mprog_name_to_type ( char *name )
{
   ...
   if ( !str_cmp( name, "emote_prog"     ) )    return EMOTE_PROG;
...
void load_area_file( AREA_DATA *tarea, char *filename )
{
	...
	else if ( !str_cmp( word, "AREA"     ) )
	{
        ...
        else if ( !str_cmp( word, "INCOME"   ) ) load_income  (tarea, fpArea);
        else if ( !str_cmp( word, "POPSUPPORT" ))load_popsupport(tarea, fpArea);
        else if ( !str_cmp( word, "GOVERNMENT" ))load_government(tarea, fpArea);
        else if ( !str_cmp( word, "GUARDS" ))load_guards(tarea, fpArea);
	...
	/* Rennard */
        ...
        else if ( !str_cmp( word, "SHIPS"    ) ) load_ships   (tarea, fpArea);
		 ...
		 char *loadst, char *notloadst )
{
    ...
    pager_printf( ch, "Areas listed: %d  Loaded: %d&w\n\r", count, loaded );
    return;
}
...
void save_sysdata( SYSTEM_DATA sys )
{
	...
	fprintf (fp, "PetSave	     %d\n", sys.save_pets		);
        ...
        fprintf( fp, "Pkloot         %d\n", sys.pk_loot );
        for ( z = 0; z < MAX_NPC_RACE; z++ )
        fprintf(fp, "Race            %d %d\n", z, sys.race_count[z]);
        for ( z = 0; z < 3; z++ )
        fprintf(fp, "Sex             %d %d\n", z, sys.sex_count[z]);
        fprintf(fp, "Pfile_count     %d\n", sys.pfile_count); 
        fprintf( fp, "End\n\n" );
        fprintf( fp, "#END\n" );
        fclose( fp );
        fp = NULL;
...
void fread_sysdata( SYSTEM_DATA *sys, FILE *fp )
{
	...
	case 'P':
	    ...
	    KEY( "Protoflag",	   sys->level_modify_proto, fread_number( fp ) );
            KEY( "PLLoss",      sys->plloss,        fread_number( fp ) );
            KEY( "Pfile_count",      sys->pfile_count,        fread_number( fp ) );
	...
	for(pArea = first_area; pArea; pArea = pArea->next)
	{
				...
				neigh = next_neigh)
		{
			...
			if(!neigh->address)
			{
				...
				STRFREE(tneigh->name);
				DISPOSE(tneigh);
				fold_area(pArea, pArea->filename, FALSE, TRUE);
				continue;
			}
...
char *centertext2(char *text, int size)
{
  char buf[MAX_STRING_LENGTH];
  char *sent;
  int i;
  int filler = (size-strlen(text))/2-1;
  sprintf(buf, " ");
