...
{
    { "General",  	"General discussion",            1,     1,     "all", DEF_INCLUDE,21, NULL, FALSE },
    { "Ideas",	  	"Suggestion for improvement",	 1,     1,     "all", DEF_NORMAL, 60, NULL, FALSE },
    { "Announce", 	"Announcements from Immortals",  0,     LEVEL_IMMORTAL, "all", DEF_NORMAL, 60, NULL, FALSE },
    { "Bugs",	  	"Typos, bugs, errors",	         1,     1,     "imm", DEF_NORMAL, 60, NULL, FALSE },
    { "Personal", 	"Personal messages",	      	 1,     1,     "all", DEF_EXCLUDE,28, NULL, FALSE }
};
...
void do_nwrite (CHAR_DATA *ch, char *argument)
{
    ...
    sprintf (buf, "\n\rYou are now %s a new note on the &W%s&w board.\n\r"
...
void do_global_boards (CHAR_DATA *ch, char *argument)
{
    ...
    if (!argument[0]) /* show boards */
    {
        ...
        for (i = 0; i < MAX_BOARD; i++)
        {
            ...
            {
                sprintf (buf, "&W%2d&z> &w%12s&z [%s%4d&z] &c%s&w\n\r",
                         count, boards[i].short_name, unread ? "&r" : "&b",
...
void personal_message (const char *sender, const char *to, const char *subject, const int 
expire_days, const char *text)
{
