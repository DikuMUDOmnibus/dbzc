...
//    CHAR_DATA *victim = NULL;
	...
	if ( !IS_NPC(ch) && xIS_SET(ch->act, PLR_FREEZE) )
	{
        ...
        if ( !IS_NPC(ch) && ch->fused_with && !xIS_SET(ch->act, PLR_FUSED))
        {
            send_to_char( "You are busy at the moment!\n\r", ch );
            return;
        }
	 ...
	 */
/*
        if ( xIS_SET(ch->act, PLR_FUSED) && xIS_SET(ch->act, PLR_INVIS2) && ch->fused_with )
        {
          if ( found && cmd->fuse == 0 )
          interpret( ch->fused_with, arg);
          else
          send_to_char( "You are not the one that controls this.\n\r", ch);
          return;
        }
*/
...
*/
        ...
        if ( xIS_SET(ch->act, PLR_FUSED) && xIS_SET(ch->act, PLR_INVIS2) && ch->fused_with )
        {
          if ( found && (cmd->fuse == 1 || cmd->fuse == 0))
          {
            if ( cmd->fuse == 1)
            xSET_BIT(ch->fused_with->act, PLR_ATTACK);

            interpret( ch->fused_with, arg);
          }
          else if ( found && cmd->fuse == 2)
          send_to_char( "You are not the one that controls this.\n\r", ch);
          return;
        }
        if ( xIS_SET(ch->act, PLR_FUSED) && ch->fused_with && !xIS_SET(ch->act, PLR_INVIS2) && !xIS_SET(ch->act, PLR_ATTACK))
        {
           if (found && cmd->fuse == 1)
           {
             send_to_char( "You are not the one that controls this.\n\r", ch);
        ...
        if (xIS_SET(ch->act, PLR_ATTACK))
        xREMOVE_BIT(ch->act, PLR_ATTACK);
    /*
     * Log and snoop.
     */
/*
...
*/
    ...
    && valid_watch(logline) )
    {
    	...
    	else if(xIS_SET(ch->pcdata->flags, PCFLAG_WATCH))
...
/*
    if ( ch->desc && ch->desc->fused_with )
    {
        sprintf( logname, "%s", ch->name);
        write_to_buffer( ch->desc->fused_with, logname, 0 );
        write_to_buffer( ch->desc->fused_with, "% ",    2 );
        write_to_buffer( ch->desc->fused_with, logline, 0 );
        write_to_buffer( ch->desc->fused_with, "\n\r",  2 );
    }
*/
...
*/
    ...
    if ( !found )
    {
	if ( !check_sskill( ch, command, argument )
        &&   !check_askill( ch, command, argument )
