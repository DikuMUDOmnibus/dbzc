...
timecheck:
    sysdata.race_count[race]++;
    sysdata.sex_count[sex]++;

   if( count == FALSE && !xIS_SET( pact, PLR_EXEMPT ) )
   {
      if ( exp < 500000 && tdiff > sysdata.newbie_purge )
      {
         if ( unlink(fname) == -1 )
	      perror( "Unlink" );
         else
         {
            ...
            snprintf( log_buf, MAX_STRING_LENGTH, "Player %s was deleted. Exceeded time limit of %d days. PL: %ld", name, days, exp );
            log_string( log_buf );
#ifdef AUTO_AUTH
...
void clear_race_count( void )
{
 int z;
 for ( z = 0; z < MAX_NPC_RACE; z++ )
    sysdata.race_count[z] = 0;

 save_sysdata( sysdata );
}

void clear_sex_count( void )
{
 int z;
 for ( z = 0; z < 3; z++ )
    sysdata.sex_count[z] = 0;
