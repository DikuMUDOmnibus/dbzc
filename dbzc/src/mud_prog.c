...
if ( !str_cmp(chck, "roomcount") )
  {
    int room_vnum = atoi(cvar);
    int lhsvl;
    CHAR_DATA *oMob;
    ...
    if ( !room_vnum )
     ...
     location = get_room_index( room_vnum );
     
     if (location == NULL)
     {
     	progbug( "mud_prog.c: Bad Location in roomcount", mob );
      return BERR;
     }
    
    
    lhsvl = 0; 
    for ( oMob = location->first_person; oMob;
...
//	if ( vnum < 1 || vnum > 32767 )
	...
	{
	   return xIS_SET(chkchar->act, PLR_FLY) ? TRUE : FALSE;
	}
	...
	if ( !str_cmp(chck, "isgood") )
	{
        ...
        if ( !str_cmp(chck, "haspropagandized") )
        {
            if (HAS_PROPAGANDIZED(chkchar) || HAS_FOUGHT(chkchar))
            return TRUE;
	...
	if ( !str_cmp(chck, "isimmort") )
	{
        ...
        if (!str_cmp (chck, "leader"))
        {
            return  is_leader(chkchar) ? TRUE : FALSE;
        }
	...
	if ( !str_cmp(chck, "inroom") )
	{
        ...
        {
            return mprog_veval(chkchar->quest, opr, atoi(rval), mob);
        }
	...
	if ( !str_cmp(chck, "level") )
	{
        ...
        if ( !str_cmp(chck, "pl") )
        {
            return mprog_veval(chkchar->exp, opr, atoi(rval), mob);
        }
        
        if ( !str_cmp( chck, "clanlevel") )
        {
           if ( IS_NPC(chkchar) || !chkchar->pcdata->clan)
             return FALSE;

           return mprog_veval(chkchar->pcdata->clan->level, opr, atoi(rval), mob);
        }
 ...
 switch ( ch ) {
     ...
     case 'c':
         if ( mob && !char_died(mob) )
         {
           if (!IS_NPC(actor))
           {
             if ( actor->pcdata && actor->pcdata->clan )
             {
               strcpy( t, actor->pcdata->clan->name );
             }
             else
             {
               strcpy( t, "None" );
     ...
     case 'f':
         if ( mob && !char_died(mob) )
         {
           if ( actor->fighting && actor->fighting->who->name )
           {
     ...
     case 'e':
         if ( actor && !char_died(actor) )
	 {
	   can_see( mob, actor,TRUE ) ? strcpy( t, he_she[ actor->sex ] )
	                         : strcpy( t, "someone" );
         } 
	 else
	      strcpy( t, "it" );
     ...
     case 'm':
         if ( actor && !char_died(actor) )
	 {
	   can_see( mob, actor, TRUE ) ? strcpy( t, him_her[ actor->sex ] )
                                 : strcpy( t, "someone" );
         }
	 else
	      strcpy( t, "it" );
     ...
     case 's':
         if ( actor && !char_died(actor) )
	 {
	   can_see( mob, actor, TRUE ) ? strcpy( t, his_her[ actor->sex ] )
	                         : strcpy( t, "someone's" );
         }
	 else
	      strcpy( t, "its'" );
     ...
     case 'E':
         if ( vict && !char_died(vict) )
	 {
	   can_see( mob, vict, TRUE ) ? strcpy( t, he_she[ vict->sex ] )
                                : strcpy( t, "someone" );
         }
	 else
	      strcpy( t, "it" );
     ...
     case 'M':
         if ( vict && !char_died(vict) )
	 {
	   can_see( mob, vict, TRUE ) ? strcpy( t, him_her[ vict->sex ] )
                                : strcpy( t, "someone" );
         }
	 else
	      strcpy( t, "it" );
     ...
     case 'S':
         if ( vict && !char_died(vict) )
	 {
	   can_see( mob, vict, TRUE ) ? strcpy( t, his_her[ vict->sex ] )
                                : strcpy( t, "someone's" ); 
         }
	 else
	      strcpy( t, "its'" );
     ...
     case 'J':
         if ( rndm && !char_died(rndm) )
	 {
	   can_see( mob, rndm, TRUE ) ? strcpy( t, he_she[ rndm->sex ] )
	                        : strcpy( t, "someone" );
         }
	 else
	      strcpy( t, "it" );
     ...
     case 'K':
         if ( rndm && !char_died(rndm) )
	 {
	   can_see( mob, rndm, TRUE ) ? strcpy( t, him_her[ rndm->sex ] )
                                : strcpy( t, "someone's" );
         }
	 else
	      strcpy( t, "its'" );
     ...
     case 'L':
         if ( rndm && !char_died(rndm) )
	 {
	   can_see( mob, rndm, TRUE ) ? strcpy( t, his_her[ rndm->sex ] )
	                        : strcpy( t, "someone" );
         }
	 else
	      strcpy( t, "its" );
...
void mprog_emote_trigger( char *txt, CHAR_DATA *ch )
{
    ...
    {
        if ( IS_NPC(vmob) && HAS_PROG(vmob->pIndexData, EMOTE_PROG) )
        {
            if ( IS_NPC(ch) && ch->pIndexData == vmob->pIndexData )
                continue;
            mprog_wordlist_check( txt, vmob, ch, NULL, NULL, EMOTE_PROG );
        }
    }
}
...
{
   if( HAS_PROG(ch->in_room, EMOTE_PROG) )
   {
        /* supermob is set and released in rprog_wordlist_check */
        rprog_wordlist_check( txt, supermob, ch, NULL, NULL, EMOTE_PROG, ch->in_room );
   }
}
...
void oprog_emote_trigger( char *txt, CHAR_DATA *ch )
{
    ...
    for ( vobj=ch->in_room->first_content; vobj; vobj = vobj->next_content )
        if ( HAS_PROG(vobj->pIndexData, EMOTE_PROG) )
            oprog_wordlist_check(txt, supermob, ch, vobj, NULL, EMOTE_PROG, vobj);
